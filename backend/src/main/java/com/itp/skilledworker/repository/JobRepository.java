...
public interface JobRepository extends JpaRepository<Job, Integer> {
    ...
    @Query("SELECT DISTINCT j FROM Job j WHERE j.customer.user.userId = :customerUserId AND EXISTS (SELECT 1 FROM JobApplication a WHERE a.job = j AND a.status <> 'rejected')")
